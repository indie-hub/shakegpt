...
/// that pair into a larger token. The ordered list of learned merges is the
/// model: encoding new text means starting from its bytes and replaying those
...
struct BPE {
    ...
    /// Trains a tokenizer by learning up to `mergeCount` frequent pairs.
    /// Training can finish early when fewer than two tokens remain.
    init(trainOn text: String, mergeCount: Int) {
        precondition(mergeCount > 0)
    ...
    func encode(_ text: String) -> [Token] {
        self.merges.reduce(byteTokens(from: text)) { tokens, pair in
            merge(pair, in: tokens)
        }
    }

    /// Reconstructs text by flattening every token back into its UTF-8 bytes.
    func decode(_ tokens: [Token]) -> String {
        let bytes = tokens.flatMap { $0 }
...
private extension BPE {
    /// Creates the initial vocabulary: one single-byte token per UTF-8 byte.
